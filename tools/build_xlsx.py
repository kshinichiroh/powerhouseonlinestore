...
RULES = [
    ("1", "チーム掲載名",
     ...
     "トーナメント表PDF・フォルダ名・ファイル名の表記が異なる場合もJBA登録名を優先する。"),
    ("2", "チーム紹介文",
    ...
    ("4", "スタッフ情報",
     "スタッフ（コーチ・帯同審判等）は誌面に掲載しない。前年度と同じく選手のみを掲載する。"
     "個人情報保護のため電話番号・メールアドレス・住所・JBAメンバーIDも入稿データに含めない。"),
    ("5", "選手名簿の項目と順序",
...
def build_tournament():
    ...
    lines = [
        ...
        ("  07_表記ゆれ", "トーナメント表PDFとJBA登録名が異なる箇所の誌面表記"),
    ...
    for gender, bracket, sheet in (("男子", BRACKET_M, "05_男子_組合せ"),
        ...
        title(ws, f"{gender} 組合せ番号とチーム名（全{len(bracket)}チーム）",
              "「誌面に印字するチーム名」はJBA帳票の登録名です。トーナメント表PDFの表記と異なる場合はこちらを正とします。")
        header(ws, 4, ["組合せ番号", "トーナメント表PDFの表記", "誌面に印字するチーム名（JBA登録名）",
                       "集合写真", "ロゴ", "広告", "備考"])
        r = 5
    ...
    title(ws, "トーナメント表PDFとJBA登録名が異なる箇所",
          "誌面に印字するチーム名は右列（JBA登録名）を使用してください。")
    header(ws, 4, ["男女", "組合せ番号", "トーナメント表PDFの表記", "誌面に印字するチーム名（JBA登録名）", "備考"])
    r = 5
    diffs = []
    for gender, bracket in (("男子", BRACKET_M), ("女子", BRACKET_W)):
        for i, b in enumerate(bracket, 1):
            ...
            if p and p != b:
                diffs.append([gender, i, b, p, "同一チーム。スペースの有無のみの違い。"])
